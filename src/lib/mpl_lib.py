    ...
    if not iarange:
        for j in range(nh):
            ...
            for i in range(nw):
                ...
                a = left + w0 + dw * i
                c = ws
                if nfig is not None and nfig == len(fig.axes):
                    pass
                else:
...
def norm_cmap(mx, mn, val=None, cm_name='jet', inorm=False):
    ...
    if val is None and inorm == False: return cmap, m
    if val is None and inorm == True:
        return cmap, m, norm
    else:
        return cmap, m.to_rgba(val)
...
def add_cb(ax, cmap=None, spacing='proportional', filled=True,
    ...
    import matplotlib as mpl
    import numpy as np
    if cmap is None: print('Warning: color map was not specified.')
