...
def reader_nc(fn='sf_unload_ph1.out',nc=481):
    ...
    for i in range(len(lines)):
        try: n0=len(map(float,lines[i].split()))
        except: 
            print 'Line %i is not pure float'%i
            pass
        else:
            if n0==nc:
                rst_lines.append(map(float,lines[i].split()))
            else:
...
def read_sf_scan(fn='sf_unload_ph1.out'):
    ...
    for ib in range(len(blocks)):
        ...
        sin2psi = map(float,blocks[ib][0].split()[7:])
        sigma = np.zeros((6,nstp))
        ehkl  = np.zeros((nc-6-1,nstp))
        for istp in range(nstp):
            sigma[:,istp] = map(float,blocks[ib][istp+2].split()[1:7])
            ehkl[:,istp]  = map(float,blocks[ib][istp+2].split()[7:])
        sig.append(sigma)
        eps.append(ehkl)
    return sig,eps
